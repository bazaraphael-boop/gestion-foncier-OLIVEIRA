...
import { X, Upload, FileCode, CheckCircle2, AlertCircle, Layers, Plus } from 'lucide-react';
import { parseKMLToGeoJSON } from '../utils/kmlParser';
import { calculateArea } from '../utils/geoUtils';
import * as turf from '@turf/turf';

export default function KmlParcelImporterModal({
  isOpen,
  onClose,
  onAddParcels,
  concessionPolygon
  ...
  const processKMLText = (text) => {
    ...
    try {
      ...
      if (!geojson || !geojson.features || geojson.features.length === 0) {
        setErrorMsg('Aucune entité géométrique (Placemark) trouvée dans le fichier KML.');
        setParsedParcels([]);
        return;
      }
      ...
      if (polygonFeatures.length === 0) {
        setErrorMsg('Le fichier KML contient des entités mais aucun polygone de parcelle valide.');
        setParsedParcels([]);
        return;
      }

      // Format parsed parcels with calculated area
      const parcels = polygonFeatures.map((feat, index) => {
        const areaInfo = calculateArea(feat);
        const name = feat.properties?.name || `Lot KML ${index + 1}`;
        const description = feat.properties?.description || '';

        return {
          type: 'Feature',
          id: `kml_parcel_${Date.now()}_${index}`,
          properties: {
            lotNumber: name.startsWith('LOT') ? name : `LOT-${name}`,
            status: defaultStatus,
            occupantName: description || '',
            notes: `Importé depuis le fichier KML ${file ? file.name : ''}`,
      ...
      setErrorMsg('Erreur lors du traitement du fichier KML : ' + err.message);
      setParsedParcels([]);
    }
  };
  ...
  const handleConfirmImport = () => {
    ...
    // Apply default status to all imported parcels
    const finalParcels = parsedParcels.map((p) => ({
      ...p,
      properties: {
        ...p.properties,
        status: defaultStatus
      ...
      <div className="bg-white text-slate-800 w-full max-w-xl rounded-lg shadow-xl border border-slate-200 overflow-hidden my-auto animate-in zoom-in-95 duration-200">
        ...
        <div className="px-5 py-3 border-b border-slate-200 flex items-center justify-between bg-slate-50">
          <div className="flex items-center gap-2 font-bold text-slate-900 text-sm">
            ...
            <span>Importer des Parcelles KML / Google Earth</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-4 text-xs">
          <p className="text-slate-600 leading-relaxed">
            Sélectionnez un fichier KML exporté depuis **Google Earth** ou **QGIS**. L'application extraira automatiquement les polygones des lots ainsi que leurs coordonnées GPS et leur surface exacte.
          </p>
          ...
          <div className="border-2 border-dashed border-slate-300 hover:border-emerald-500 bg-slate-50 p-6 rounded-lg text-center cursor-pointer transition-all relative">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="font-bold text-slate-800">
                {file ? file.name : 'Cliquez ou glissez-déposez un fichier KML ici'}
              </span>
              <span className="text-[11px] text-slate-400">
                Formats acceptés : `.kml` ou `.xml`
              </span>
            </div>
          </div>
          ...
          {parsedParcels.length > 0 && (
            <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3 rounded border border-slate-200">
              ...
              <div className="flex flex-col justify-center">
                <span className="text-[11px] text-slate-500 font-medium">Nombre de Lots Identifiés</span>
          ...
          {parsedParcels.length > 0 && (
            <div className="space-y-1.5">
              <span className="font-bold text-slate-800 text-xs block">Aperçu des Parcelles Détectées :</span>
        ...
        <div className="px-5 py-3 border-t border-slate-200 bg-slate-50 flex justify-end gap-2 text-xs">
          ...
          >
            ...
            <span>Importer {parsedParcels.length} Parcelles</span>
          </button>
        </div>
      </div>
    </div>
